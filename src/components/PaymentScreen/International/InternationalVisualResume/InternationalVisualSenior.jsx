...
const InternationalVisualEntry = () => {
  ...
  const [seniorLevel, setseniorLevel] = useState();

  useEffect(()=>{
    const fetchData=async()=>{
      setLoading(true);
      try {
        const response=await axios.get(`${process.env.REACT_APP_SERVER_PRO_URL}/api/visualinternationalresume/plans`);
        const { status, plans } = response.data;
        if (status === "success" && Array.isArray(plans) && plans.length > 0){
          setseniorLevel(plans[2]);
        }
      } catch (error) {
